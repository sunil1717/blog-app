...
export default function SearchClient({ queryParam }) {
  ...
  if (!queryBlogs || queryBlogs.length === 0)
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-white p-5 rounded-xl shadow">
              ...
              <div className="space-y-4">
                {recentBlogs?.map((b) => (
                  <Link key={b.slug} href={`/${b.slug}`} className="flex gap-4 items-center group">
                    {b.images?.[0] && (
                      <img
                        src={b.images[0]}
                        alt={b.title}
                    ...
                    <div>
                      ...
                      <p className="text-gray-500 text-xs">{new Date(b.publishedDate).toLocaleDateString()}</p>
                    </div>
                  </Link>
                ))}
              </div>
